...
router.post('/excel_users', uploadExcel.single('file'), async function (req, res) {
    if (!req.file) return res.send({ message: "file khong duoc rong" });
    ...
    try {
        ...
        if (!roleUser) {
            return res.status(500).send({ message: "he thong khong co role 'user'" });
        }

        for (let index = 2; index <= worksheet.rowCount; index++) {
            const row = worksheet.getRow(index);
            
            const getVal = (cell) => (cell.value && cell.value.result) ? cell.value.result : cell.value;
            
            let username = getVal(row.getCell(1));
            let email = getVal(row.getCell(2));
            ...
            try {
                ...
                const gitUrl = "https://github.com/nguyenthanhtunghutechsg/NNPTUD-S3";
                const infoForUser = `${gitUrl}?password=${plainPassword}`;
                await emailHandler.sendMail(email, infoForUser);
                ...
                result.push({
                    row: index,
                    success: false,
                    error: err.message.includes('duplicate') ? "mail hoac username da ton tai" : err.message
                });
            }
        }

        fs.unlinkSync(filePath);
        res.send({ message: "done", chi_tiet: result });
